...
This endpoint allows a resident to block/unblock an agent's communication:contact_occupant scope:
...
<done>Domain entity exports types + 3 hooks; block endpoint narrows communication:contact_occupant; audit endpoint returns chronological log; typechecks clean</done>
</task>
...
| agent → blocked scope   | Block removes communication:contact_occupant from AgentAccess.permissions. Scope narrowed server-side, not client. |
...
| T-111-19  | Elevation of Privilege | Block scope manipulation | mitigate    | Block removes communication:contact_occupant only. Unblock restores it only if present in originalPermissions — the grant-time snapshot. Agent scope can never exceed the original delegation ceiling regardless of block/unblock sequence. |
...
4. Toggle block on an agent → agent's communication:contact_occupant scope removed server-side
...
- Per-agent block toggle removes communication:contact_occupant server-side (narrowed scope, not client-side)
