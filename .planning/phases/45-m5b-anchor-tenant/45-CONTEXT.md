...
- **D-03:** Tier thresholds (Gold/Silver/Bronze/Watchlist/Probation) default to hardcoded constants (Soralia defaults). `getStandingTier()` accepts an optional `thresholds` override so tenant settings can customize later. For MVP, only defaults are used — no tenant-settings admin UI. Re-evaluate when a second tenant needs different thresholds.
...
- **D-07:** Email-based OTP (6-digit code) via Better Auth `emailOTP` plugin. No authenticator app required — works with existing Resend infrastructure (Resend SDK `^6.12.2`, `src/shared/api/email/resend.ts`). Lower friction for residents.
...
- Email OTP follows the same email infrastructure as Phase 10 (Resend) — no new email provider.
  </specifics>
...
- **Per-tenant configurable tier thresholds** — `getStandingTier()` API supports optional override, but no admin UI for editing thresholds in MVP. If a second tenant needs different thresholds, add a settings panel.
