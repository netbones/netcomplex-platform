...
function initDashboardMap() {
    ...
    const communityLocation = [-34.091650, 18.483269];
    ...
    }).addTo(dashboardMap).bindPopup('<b>Your Home</b><br>123 Pagoda Rd');
    ...
    }).addTo(dashboardMap).bindPopup('<b>Community Center</b><br>Events & Meetings');
    ...
    maintenanceIssues.forEach(issue => {
        const color = issue.status === 'in-progress' ? 'yellow' : 
                     issue.status === 'completed' ? 'green' : 'red';
        ...
        }).addTo(dashboardMap).bindPopup(`<b>${issue.title}</b><br>${issue.description}<br><em>Status: ${issue.status}</em>`);
    });
}
...
function openBookingSystem() {
    alert('Booking system would open here. Features:\n\n• Book community center\n• Reserve garden plots\n• Schedule facility maintenance\n• View availability calendar');
}

function openPayments() {
    alert('Payment portal would open here. Features:\n\n• View monthly statements\n• Pay community fees\n• Set up automatic payments\n• Download receipts');
}
