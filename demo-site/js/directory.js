...
function filterDirectory() {
  ...
  filteredResidents = residents.filter(resident => {
    ...
    const matchesStreet =
      !streetFilter || resident.address.toLowerCase().includes(streetFilter.replace('-', ' '));
...
function renderDirectory() {
  ...
  if (currentView === 'grid') {
    gridView.innerHTML = filteredResidents
      .map((resident, index) => createResidentCard(resident, index))
      .join('');
    listView.innerHTML = '';
  } else {
    listView.innerHTML = filteredResidents
      .map(resident => createResidentListItem(resident))
      .join('');
    gridView.innerHTML = '';
  }
