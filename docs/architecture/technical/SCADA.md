a lightweight SCADA/HMI (human-machine interface) product on top of your multi-tenant platform. The key insight is that the data flows in from the physical world through a narrow ingestion funnel, then fans out across your existing tenant, notification, and permission architecture. Most of the complexity lives at the edges — the field hardware and the UX — not in your application code.
...
3. Data storage — time-series in Supabase
   Your existing Supabase/PostgreSQL setup can handle this with the TimescaleDB extension, which Supabase supports. TimescaleDB adds hypertables (auto-partitioned by time), compression, and downsampling functions on top of standard Postgres — so your existing Drizzle/Prisma tooling still works for the metadata, and you get efficient time-series queries for readings.
...
The IoTZone and IoTDevice models are metadata — managed via your existing tRPC/admin flows. IoTReading is the high-volume table that becomes a TimescaleDB hypertable. You'd set a retention policy (e.g. keep raw readings for 30 days, downsample to hourly averages for 1 year) so it doesn't grow unboundedly.

4. Alert engine
   Since you're on Vercel (no persistent processes), alert evaluation fits into one of two places:
...
5. The dashboard UI
   Following your widget-first principle from AGENTS.md, this should be a new IoTMonitorWidget — not a new page — that renders inside the existing dashboard. It would contain:

A Leaflet map (you already have react-leaflet in the stack) with zone polygons overlaid, coloured by status (green/amber/red).
...
6. Gating it as a premium feature
   Your PlatformModule + TenantModule architecture already handles this perfectly. You'd seed:
   typescript{
   key: "iot_monitor",
   ...
   Every API route and component that touches IoT data checks tenantModule.enabled for iot_monitor, exactly like your other gated modules. The TenantModule.config JSON field is useful here too — you could store per-tenant gateway credentials, LNS webhook secrets, and default alert recipient roles in there without adding more columns.
...
Phase 1 can be done without any hardware — you can simulate device payloads by posting JSON directly to your ingest route. That way the backend is production-ready before a single gateway is deployed.
