...
**Advisory number:** PENDING CONFIRMATION against the external advisory register. Last
confirmed number in this session's context is ADVISORY-028 (tenant directory/showcase).
This document is numbered ADVISORY-029 provisionally — confirm against the register
before the agent creates the file at `docs/advisories/ADVISORY-029.md`.
