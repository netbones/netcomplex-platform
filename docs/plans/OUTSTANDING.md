...
| **Phase 38**      | Domain grid 'Communication' heading hardcoded                                                                                           | Resolved — all domain headings had i18n keys since Phase 45. Added missing `education` + `disputes` domain keys to all 4 locale files. | ✅ Resolved           |
| **Phase 39**      | Pre-existing `createCallerFactory` tRPC v11 error in template file                                                                      | Outdated template deleted — `docs/prompts/trpc_caller_test_template.ts` removed. Working patterns exist in `src/test/`.                | ✅ Resolved           |
...
| **Phase 120**     | `competitionDto` creation — competitions use inline Zod output schemas with BD-21 TODO markers                                          | DTO exists at `src/shared/api/dto/competition.ts`. Inline schemas remain due to Zod v3/v4 interop — blocked until ADR-026 resolution.  | Blocked (Zod interop) |
| **Phase 120**     | `userAchievementDto` missing — `getUnlocked` returns raw DB rows via `toEnvelope()`                                                     | Resolved — DTO created at `src/shared/api/dto/user-achievement.ts`, wired into `getUnlocked` handler.                                  | ✅ Resolved           |
