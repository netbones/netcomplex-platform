...
import { useProviderCreditScore } from './provider-queries';

export function ProviderCreditProgressWidget() {
  const { data, isLoading, error } = useProviderCreditScore();
  ...
  if (error || !data) {
    return <div className="rounded-xl bg-red-50 p-4 text-sm text-red-700">Unable to load provider credit progress.</div>;
  }

  return (
    <ProviderSection
      title="Verification progress"
      description="Credit score progress toward verified provider status."
    >
      <div className="space-y-4">
        <div className="flex items-baseline justify-between gap-4">
          <div>
            <p className="text-3xl font-semibold text-gray-900">{data.creditScore}</p>
            <p className="text-sm text-gray-500">of {data.progress.verificationThreshold} points needed</p>
          </div>
