...
interface HouseholdsWidgetProps {
  households: Household[];
  loading?: boolean;
}

export function HouseholdsWidget({ households, loading = false }: HouseholdsWidgetProps) {
  ...
  if (households.length === 0) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-gray-500 text-sm">{t('noHouseholds', 'No properties registered')}</p>
        <p className="text-gray-400 text-xs mt-1">Property owners can add their households</p>
      </div>
    );
  }
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                {household.standardSeats.some(s => s.isPrimaryOwner) && (
              ...
              <p className="text-xs text-gray-400 mt-1 font-mono">{household.platformAddress}</p>
              {household.profiles.length > 0 && (
                <div className="flex items-center gap-1 mt-2">
                  <span className="text-xs text-gray-500">
                    {household.profiles.length} {t('members', 'members')}
                  </span>
