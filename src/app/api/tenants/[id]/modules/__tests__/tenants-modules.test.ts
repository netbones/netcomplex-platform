...
describe('Tenant Modules API', () => {
  ...
  describe('GET /api/tenants/[id]/modules', () => {
    ...
    it('returns enabled modules for PREMIUM tenant', async () => {
      mocks.dbMock.select
        .mockReturnValueOnce(
          makeSelectChain([{ id: 'tenant-1', name: 'Soralia', tier: TIERS.PREMIUM }])
        )
        .mockReturnValueOnce(
          makeSelectChain([
            {
        ...
        .mockReturnValueOnce(
          makeSelectChain([{ moduleKey: 'chat', enabled: false, config: null, enabledAt: null }])
        );
      ...
      expect((body as any).data.events).toEqual({
        enabled: false,
        config: undefined,
        enabledAt: undefined,
      });
    ...
    it('respects defaultEnabled when no explicit tenant module', async () => {
      mocks.dbMock.select
        .mockReturnValueOnce(
          makeSelectChain([{ id: 'tenant-1', name: 'Soralia', tier: TIERS.STANDARD }])
        )
        .mockReturnValueOnce(
          makeSelectChain([
            {
    ...
    it('excludes modules above tenant tier', async () => {
      mocks.dbMock.select
        .mockReturnValueOnce(
          makeSelectChain([{ id: 'tenant-1', name: 'Basic', tier: TIERS.STANDARD }])
        )
        .mockReturnValueOnce(
          makeSelectChain([
            {
