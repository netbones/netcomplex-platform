...
describe('Content Moderate API', () => {
  ...
  describe('PATCH /api/content/[id]/moderate', () => {
    ...
    it('returns 400 for invalid moderation status', async () => {
      const res = await PATCH(patchRequest({ moderationStatus: 'INVALID' }), {
        params: Promise.resolve({ id: 'cont-1' }),
      });
    ...
    it('moderates content to DRAFT', async () => {
      ...
      const res = await PATCH(patchRequest({ moderationStatus: 'DRAFT' }), {
        params: Promise.resolve({ id: 'cont-1' }),
      });
      const body = await res.json();
    ...
    it('moderates content to FLAGGED', async () => {
      ...
      const res = await PATCH(patchRequest({ moderationStatus: 'FLAGGED' }), {
        params: Promise.resolve({ id: 'cont-1' }),
      });
      const body = await res.json();
    ...
    it('sets published=false when status is not PUBLISHED', async () => {
      ...
      await PATCH(patchRequest({ moderationStatus: 'UNPUBLISHED' }), {
        params: Promise.resolve({ id: 'cont-1' }),
      });
