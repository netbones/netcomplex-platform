...
import { makeUpdateChain } from '@/test/api/helpers';
...
vi.mock('@api/server', async () => {
  ...
  return {
    ...
    apiGone: (message = 'Resource gone') =>
      NextResponse.json(
        { success: false, error: { code: 'GONE', message } },
        { status: 410 }
      ) as any,
...
vi.mock('@/shared/api/auth-utils', () => ({
  requireAuth: vi.fn(async () => {
    ...
    return { success: true as const, data: { ...authData, suspension: null } };
  }),
}));
...
describe('Invitations [id] API — DELETE', () => {
  ...
  it('calls now() for deletedAt timestamp', async () => {
    await DELETE(new Request('http://localhost:3000/api/invitations/inv-1', { method: 'DELETE' }), {
