...
describe('GET /api/community-services/listings/[id]', () => {
  it('returns 404 when listing not found', async () => {
    ...
    const response = await GET(req('http://localhost:3000/api/community-services/listings/nonexistent'), params('nonexistent'));
  ...
  it('returns published listing without auth (public access)', async () => {
    ...
    const response = await GET(req('http://localhost:3000/api/community-services/listings/listing-1'), params('listing-1'));
...
describe('PUT /api/community-services/listings/[id]', () => {
  it('returns 401 without authentication', async () => {
    const response = await PUT(req('http://localhost:3000/api/community-services/listings/listing-1', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 404 when listing does not exist', async () => {
    ...
    const response = await PUT(req('http://localhost:3000/api/community-services/listings/nonexistent', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'Updated Title' }),
    }), params('nonexistent'));
  ...
  it('returns 410 when listing has been soft-deleted', async () => {
    mocks.sessionResult = { user: { id: 'provider-1' } };
    const chain = makeFullSelectChain([{ providerId: 'provider-1', deletedAt: '2026-06-20T00:00:00.000Z' }]);
    mocks.dbMock.select.mockReturnValue(chain);
  ...
  it('updates listing when user is the owner', async () => {
    ...
    mocks.dbMock.select.mockImplementation(() => {
      callIdx++;
      if (callIdx === 1) return makeFullSelectChain([{ providerId: 'provider-1', deletedAt: null }]);
...
describe('DELETE /api/community-services/listings/[id]', () => {
  it('returns 401 without authentication', async () => {
    const response = await DELETE(req('http://localhost:3000/api/community-services/listings/listing-1', {
      method: 'DELETE',
    }), params('listing-1'));
    expect(response.status).toBe(401);
  });
  ...
  it('returns 404 when listing does not exist', async () => {
    ...
    const response = await DELETE(req('http://localhost:3000/api/community-services/listings/nonexistent', {
      method: 'DELETE',
    }), params('nonexistent'));
    expect(response.status).toBe(404);
  });
