...
vi.mock('@api/server', () => ({
  ...
  db: mocks.dbMock,
  communityServiceListings: { id: 'id', tenantId: 'tenantId', providerId: 'providerId', title: 'title', description: 'description', category: 'category', __brand: 'table' },
  communityServiceInquiries: { id: 'id', listingId: 'listingId', inquirerId: 'inquirerId', tenantId: 'tenantId', serviceType: 'serviceType', preferredDate: 'preferredDate', preferredTime: 'preferredTime', location: 'location', description: 'description', contactMethod: 'contactMethod', status: 'status', providerResponse: 'providerResponse', respondedAt: 'respondedAt', createdAt: 'createdAt', updatedAt: 'updatedAt', __brand: 'table' },
...
describe('GET /api/community-services/provider/inquiries', () => {
  it('returns 401 without authentication', async () => {
    const response = await GET(req('http://localhost:3000/api/community-services/provider/inquiries/inquiry-1'));
  ...
  it('filters by status query param', async () => {
    ...
    const response = await GET(req('http://localhost:3000/api/community-services/provider/inquiries/inquiry-1?status=RESPONDED'));
    expect(response.status).toBe(200);
  });
  ...
  it('supports pagination with limit and offset', async () => {
    ...
    const response = await GET(req('http://localhost:3000/api/community-services/provider/inquiries/inquiry-1?limit=10&offset=0'));
    expect(response.status).toBe(200);
    const body = await response.json();
...
describe('POST /api/community-services/provider/inquiries/[id] (respond)', () => {
  it('returns 401 without authentication', async () => {
    const response = await POST(req('http://localhost:3000/api/community-services/provider/inquiries/inquiry-1', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 404 when inquiry not found', async () => {
    ...
    const response = await POST(req('http://localhost:3000/api/community-services/provider/inquiries/nonexistent', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
