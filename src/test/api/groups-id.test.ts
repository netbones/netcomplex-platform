...
const mocks = vi.hoisted(() => ({
  ...
  apiNotFound: vi.fn((message = 'Not found'): Response =>
    new Response(
  ...
  apiGone: vi.fn((message = 'This record has been deleted'): Response =>
    new Response(
      JSON.stringify({ success: false, error: { code: 'GONE', message } }),
...
vi.mock('@api/server', () => ({
  ...
  groups: {
    ...
    updatedAt: 'updatedAt', ownerId: 'ownerId', tenantId: 'tenantId', deletedAt: 'deletedAt',
  },
  users: { id: 'id', role: 'role', name: 'name', image: 'image' },
  groupMembers: {
    id: 'id', userId: 'userId', groupId: 'groupId', role: 'role',
    joinedAt: 'joinedAt', deletedAt: 'deletedAt',
  },
  contents: {
    id: 'id', title: 'title', content: 'content', excerpt: 'excerpt',
    category: 'category', authorId: 'authorId', groupId: 'groupId',
    published: 'published', featured: 'featured', priority: 'priority',
...
describe('GET /api/groups/[id]', () => {
  ...
  it('returns 200 with group, owner, members, and contents', async () => {
    const group = {
      id: 'g-1', name: 'Book Club', description: 'A book club', category: 'SOCIAL',
      image: null, color: '#4F46E5', isPublic: true, accessType: 'OPEN',
      residentFilter: 'ALL', isActive: true, ownerId: 'user-1',
    ...
    const contents = [
      {
        id: 'c-1', title: 'Meeting Notes', content: 'Notes...', excerpt: null,
        category: 'DISCUSSION', authorId: 'user-1', groupId: 'g-1',
        published: true, featured: false, priority: 0,
