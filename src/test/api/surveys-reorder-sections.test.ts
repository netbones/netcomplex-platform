...
describe('POST /api/surveys/[id]/sections/reorder', () => {
  ...
  it('returns 401 without auth', async () => {
    const res = await POST(makeRequest({ items: [{ id: 'sec-1', order: 1 }] }), makeParams(SURVEY_ID));
  ...
  it('returns 404 when section IDs not found in survey', async () => {
    ...
    const res = await POST(
      makeRequest({ items: [{ id: 'sec-1', order: 1 }, { id: 'sec-2', order: 2 }] }),
      makeParams(SURVEY_ID)
    );
    expect(res.status).toBe(404);
  });
  ...
  it('reorders single section', async () => {
    ...
    const res = await POST(makeRequest({ items: [{ id: 'sec-1', order: 3 }] }), makeParams(SURVEY_ID));
    const body = await res.json();
