...
const mocks = vi.hoisted(() => ({
  ...
  apiForbidden: vi.fn((message = 'Forbidden') =>
    new Response(
      JSON.stringify({ success: false, error: { code: 'FORBIDDEN', message } }),
      { status: 403, headers: { 'Content-Type': 'application/json' } }
    )
  ),
  apiNotFound: vi.fn((message = 'Not found') =>
    new Response(
      JSON.stringify({ success: false, error: { code: 'NOT_FOUND', message } }),
...
vi.mock('@api/server', async () => {
  ...
  return {
    ...
    maintenanceRequests: {
      ...
      status: 'status', images: 'images', assignedTo: 'assignedTo', vendor: 'vendor',
      scheduledDate: 'scheduledDate', estimatedCost: 'estimatedCost',
      actualCost: 'actualCost', resolution: 'resolution', completedAt: 'completedAt',
      createdAt: 'createdAt', updatedAt: 'updatedAt', ticketNumber: 'ticketNumber',
    ...
    requestHistories: {
      id: 'id', requestId: 'requestId', userId: 'userId', field: 'field',
      oldValue: 'oldValue', newValue: 'newValue', comment: 'comment', createdAt: 'createdAt',
    },
...
describe('GET /api/maintenance/[id]', () => {
  ...
  it('returns full response with team and provider for admin', async () => {
    ...
    mocks.dbMock.select
      ...
      .mockReturnValueOnce(makeSelectChain([{ id: 'prov-1', companyName: 'FixIt Co', trade: 'PLUMBING' }]));
    ...
    expect(body.data.assignedProvider).toEqual({ id: 'prov-1', companyName: 'FixIt Co', trade: 'PLUMBING' });
...
describe('PATCH /api/maintenance/[id]', () => {
  beforeEach(() => {
    ...
    mocks.dbMock.insert.mockReturnValue({ values: vi.fn(() => Promise.resolve([{ id: 'hist-1' }])) });
  ...
  it('sets completedAt when transitioning to COMPLETED', async () => {
    ...
    const updated = { ...mockRequest, status: 'COMPLETED', completedAt: mocks.nowDate, updatedAt: mocks.nowDate };
  ...
  it('updates estimatedCost and actualCost', async () => {
    ...
    const updated = { ...mockRequest, estimatedCost: 100, actualCost: 95, updatedAt: mocks.nowDate };
    mocks.dbMock.update.mockReturnValue(makeUpdateChain([updated]));

    const res = await PATCH(makeRequest('PATCH', { estimatedCost: 100, actualCost: 95 }), makeParams());
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('updates scheduledDate and creates history entry', async () => {
    ...
    const res = await PATCH(makeRequest('PATCH', { scheduledDate: '2026-06-28T09:00:00Z' }), makeParams());
    expect(res.status).toBe(200);
    expect(mocks.dbMock.insert).toHaveBeenCalled();
  });
