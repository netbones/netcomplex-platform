...
describe('Users Books API', () => {
  ...
  describe('GET /api/users/[id]/books', () => {
    ...
    it('allows ADMIN to view any user\'s books', async () => {
      ...
      mocks.dbMock.select.mockReturnValueOnce(makeSelectChain([{ books: [{ id: 'book-1', title: 'Test Book' }] }]));
    ...
    it('allows a user to view their own books', async () => {
      mocks.dbMock.select.mockReturnValueOnce(makeSelectChain([{ books: [{ id: 'book-1', title: 'My Book' }] }]));
  ...
  describe('POST /api/users/[id]/books', () => {
    ...
    it('returns 401 without auth session', async () => {
      ...
      const res = await POST(postRequest({ action: 'add', book: { id: 'b1', title: 'New Book' } }), {
        params: Promise.resolve({ id: 'user-1' }),
      });
    ...
    it('adds a book to the user\'s list', async () => {
      ...
      mocks.dbMock.select.mockReturnValueOnce(makeSelectChain([{ books: [{ id: 'b0', title: 'Old Book' }] }]));
    ...
    it('allows ADMIN to add books to another user', async () => {
      ...
      const res = await POST(postRequest({ action: 'add', book: { id: 'b1', title: 'Admin Added' } }), {
        params: Promise.resolve({ id: 'user-1' }),
      });
      const body = await res.json();
