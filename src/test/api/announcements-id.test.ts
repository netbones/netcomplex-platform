...
const mocks = vi.hoisted(() => ({
  ...
  apiError: vi.fn((code: string, message: string, status = 400): Response =>
...
describe('GET /api/announcements/[id]', () => {
  ...
  it('passes the id from params into the query', async () => {
    ...
    const response = await GET(makeReq({ url: 'http://localhost:3000/api/announcements/custom-1' }), {
...
describe('PATCH /api/announcements/[id]', () => {
  ...
  it('accepts partial updates', async () => {
    ...
    mocks.dbMock.update.mockReturnValue(
      makeUpdateChain([{ id: 'ann-1', author: 'New Author' }])
    );

    const response = await PATCH(
      makeReq({ method: 'PATCH', body: { author: 'New Author' } }),
      { params: Promise.resolve({ id: 'ann-1' }) }
    );
    expect(response.status).toBe(200);
  });
