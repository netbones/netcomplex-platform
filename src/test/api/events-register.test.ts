...
vi.mock('@api/server', () => ({
  ...
  apiUnauthorized: vi.fn((message?: string) =>
    Response.json(
      { success: false, error: { code: 'AUTH_REQUIRED', message: message || 'Authentication required' } },
      { status: 401 }
    )
  ),
...
describe('GET /api/events/[id]/register', () => {
  ...
  it('returns attendee list with registered=true when user is attending', async () => {
    ...
    const attendeeChain = makeSelectChain([
      { id: 'a-1', userId: USER_ID, name: 'Alice', avatar: null, createdAt: new Date().toISOString() },
      { id: 'a-2', userId: 'user-2', name: 'Bob', avatar: null, createdAt: new Date().toISOString() },
    ]);
  ...
  it('returns attendee list with registered=false when user is not attending', async () => {
    ...
    const attendeeChain = makeSelectChain([
      { id: 'a-3', userId: 'user-2', name: 'Bob', avatar: null, createdAt: new Date().toISOString() },
    ]);
    mocks.dbMock.select.mockReturnValue(attendeeChain);
...
describe('POST /api/events/[id]/register', () => {
  ...
  it('registers user and returns success', async () => {
    ...
    const newAttendee = { id: 'a-new', eventId: EVENT_ID, userId: USER_ID, tenantId: 'test-tenant-id' };
