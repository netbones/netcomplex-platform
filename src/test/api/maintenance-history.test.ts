...
vi.mock('@api/server', async () => {
  ...
  return {
    ...
    db: mocks.dbMock,
    requestHistories: {
      id: 'id', requestId: 'requestId', field: 'field',
      oldValue: 'oldValue', newValue: 'newValue', comment: 'comment',
...
describe('POST /api/maintenance/[id]/history', () => {
  ...
  it('returns 401 without auth', async () => {
    const res = await POST(makeRequest('POST', { field: 'status', newValue: 'ASSIGNED' }), makeParams());
    expect(res.status).toBe(401);
  });
  ...
  it('returns 201 and creates history entry with all fields', async () => {
    ...
    const res = await POST(
      makeRequest('POST', { field: 'priority', oldValue: 'HIGH', newValue: 'LOW', comment: 'Reducing priority' }),
      makeParams()
    );
