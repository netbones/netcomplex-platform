...
vi.mock('@api/server', () => ({
  ...
  db: mocks.dbMock,
  questions: { id: 'id', surveyId: 'surveyId', tenantId: 'tenantId', order: 'order', sectionId: 'sectionId' },
...
describe('POST /api/surveys/[id]/questions/reorder', () => {
  ...
  it('returns 401 without auth', async () => {
    const res = await POST(makeRequest({ items: [{ id: 'q-1', order: 1 }] }), makeParams(SURVEY_ID));
  ...
  it('returns 404 when question IDs not found in survey', async () => {
    ...
    const res = await POST(
      makeRequest({ items: [{ id: 'q-1', order: 1 }, { id: 'q-2', order: 2 }] }),
      makeParams(SURVEY_ID)
    );
    expect(res.status).toBe(404);
  });
