...
const mocks = vi.hoisted(() => ({
  ...
  hasPermission: vi.fn(),
  apiSuccess: vi.fn((data: unknown): Response =>
    new Response(JSON.stringify({ success: true, data }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    })
  ),
  apiUnauthorized: vi.fn((message = 'Authentication required'): Response =>
    new Response(
      JSON.stringify({ success: false, error: { code: 'AUTH_REQUIRED', message } }),
      { status: 401, headers: { 'content-type': 'application/json' } }
    )
  ),
  apiForbidden: vi.fn((message = 'Forbidden'): Response =>
    new Response(
      JSON.stringify({ success: false, error: { code: 'FORBIDDEN', message } }),
      { status: 403, headers: { 'content-type': 'application/json' } }
    )
  ),
  apiError: vi.fn((code: string, message: string, status = 500): Response =>
...
vi.mock('@api/server', () => ({
  ...
  groupMembershipRequests: {
    id: 'id', userId: 'userId', groupId: 'groupId', status: 'status',
    message: 'message', createdAt: 'createdAt', tenantId: 'tenantId',
  },
...
describe('GET /api/groups/membership-requests', () => {
  ...
  describe('authentication and authorization', () => {
    ...
    it('allows access with content permission', async () => {
      ...
      // First select call is from getSessionAndRole (user role lookup)
      // Second select call is the main membership request query
      mocks.dbMock.select
  ...
  describe('query parameter filtering', () => {
    it('defaults status filter to PENDING when no status query param', async () => {
      ...
      const requests = [
        {
          id: 'req-1', userId: 'user-2', groupId: 'g-1', status: 'PENDING',
          message: 'Please add me', createdAt: new Date(),
    ...
    it('filters by groupId when query param is provided', async () => {
      ...
      mocks.dbMock.select
        ...
        .mockReturnValueOnce(makeSelectChain([
          {
            id: 'req-1', userId: 'user-2', groupId: 'g-1', status: 'PENDING',
            message: null, createdAt: new Date(),
            user: { name: 'Bob', email: 'bob@test.com' },
            group: { name: 'Book Club', accessType: 'OPEN' },
          },
        ]));
    ...
    it('combines status and groupId filters', async () => {
      ...
      const response = await GET(
        makeReq(
          'http://localhost:3000/api/groups/membership-requests?status=PENDING&groupId=g-1'
        )
      );
      expect(response.status).toBe(200);
      expect(response.status).toBe(200);
  ...
  describe('data shape and edge cases', () => {
    it('returns requests with user and group join data', async () => {
      ...
      const requests = [
        {
          id: 'req-1', userId: 'user-2', groupId: 'g-1', status: 'PENDING',
          message: 'I would like to join', createdAt: new Date('2026-06-21T10:00:00Z'),
    ...
    it('handles null user/group joins gracefully', async () => {
      ...
      const requests = [
        {
          id: 'req-1', userId: 'deleted-user', groupId: 'deleted-group', status: 'PENDING',
          message: null, createdAt: new Date(),
          user: null,
          group: null,
        },
    ...
    it('orders results by createdAt descending', async () => {
      ...
      const requests = [
        {
          id: 'req-1', status: 'PENDING', createdAt: new Date('2026-06-21T12:00:00Z'),
          user: null, group: null,
        },
        {
          id: 'req-2', status: 'PENDING', createdAt: new Date('2026-06-20T12:00:00Z'),
          user: null, group: null,
        },
      ];
  ...
  describe('tenant isolation', () => {
    it('scopes requests to current tenant', async () => {
      ...
      const selectChain = mocks.dbMock.select.mock.results[1]?.value;
