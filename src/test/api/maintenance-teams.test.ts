...
const mocks = vi.hoisted(() => ({
  ...
  apiError: vi.fn((_code: string, _message: string, status: number) =>
    new Response(
      JSON.stringify({ success: false, error: { code: _code, message: _message } }),
...
vi.mock('@api/server', async () => {
  ...
  return {
    db: mocks.dbMock,
    maintenanceTeams: {
      id: 'id', tenantId: 'tenantId', name: 'name', trade: 'trade',
      contactName: 'contactName', isActive: 'isActive',
...
import { GET, POST } from '@/app/api/maintenance/teams/route';

describe('GET /api/maintenance/teams', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('returns 401 when auth fails', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/maintenance/teams') as any);
  ...
  it('returns list of teams when authorized', async () => {
    const teams = [
      { id: 't-1', tenantId: 'test-tenant-id', name: 'Plumbers', trade: 'PLUMBING', contactName: 'Alice', isActive: true, createdAt: new Date(), updatedAt: new Date(), deletedAt: null },
      { id: 't-2', tenantId: 'test-tenant-id', name: 'Electricians', trade: 'ELECTRICAL', contactName: 'Bob', isActive: true, createdAt: new Date(), updatedAt: new Date(), deletedAt: null },
    ];
    mocks.dbMock.select.mockReturnValue(makeSelectChain(teams));
    ...
    expect((body as any).data[0].name).toBe('Plumbers');
  });

  it('filters teams by isActive=true query param', async () => {
    mocks.dbMock.select.mockReturnValue(makeSelectChain([]));

    await GET(new Request('http://localhost/api/maintenance/teams?isActive=true') as any);

    expect(mocks.dbMock.select).toHaveBeenCalled();
  });

  it('filters teams by isActive=false query param', async () => {
    mocks.dbMock.select.mockReturnValue(makeSelectChain([]));

    await GET(new Request('http://localhost/api/maintenance/teams?isActive=false') as any);
...
describe('POST /api/maintenance/teams', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.requireAnyPermission.mockResolvedValue(null);
    mocks.dbMock.select.mockReturnValue(makeSelectChain([]));
  ...
  it('returns 401 when auth fails', async () => {
    ...
    const res = await POST(
      new Request('http://localhost/api/maintenance/teams', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: 'Plumbers', trade: 'PLUMBING' }),
      })
    );
    expect(res.status).toBe(401);
  });
  ...
  it('returns 400 when trade is missing', async () => {
    const res = await POST(
      new Request('http://localhost/api/maintenance/teams', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: 'Plumbers' }),
      })
    );
  ...
  it('returns 400 when both name and trade are missing', async () => {
    const res = await POST(
      new Request('http://localhost/api/maintenance/teams', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  it('creates a team successfully with required fields', async () => {
    const created = {
      id: 't-new',
    ...
    expect((body as any).data.name).toBe('Plumbers');
    expect((body as any).data.trade).toBe('PLUMBING');
    expect((body as any).data.isActive).toBe(true);
  });

  it('creates a team with optional contactName', async () => {
    const created = {
      id: 't-new',
      tenantId: 'test-tenant-id',
      name: 'Plumbers',
    ...
    const res = await POST(
      new Request('http://localhost/api/maintenance/teams', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: 'Plumbers', trade: 'PLUMBING', contactName: 'Alice Smith' }),
      })
    );
    const body = await res.json();
