...
2. **Add the outbox + dead-letter tables** (migration) carrying the envelope fields; add an `emitDomainEvent(type, payload, ctx)` that writes to outbox (atomic with the business write) and, interim, still calls the in-process bus for same-request handlers.
3. **Build the Outbox Dispatcher** (Supabase Realtime fan-out or cron worker) that reads the registry and fans out to `registerHandler` consumers.
4. **Port listeners** from `onEvent` to `registerHandler`; delete `emitter.ts`'s `EventEmitter` once the dispatcher owns dispatch.
5. **Wrap Realtime** in `lib/realtime.ts` with typed, namespaced channels; migrate existing `postgres_changes`/`broadcast` call sites.
